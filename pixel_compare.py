...
# === ПУТИ ===
BASE_DIR = Path(r"C:\Users\8twii\PycharmProjects\test_task_vanger_rf")
...
def capture_actual(baseline_path: Path, out_path: Path):
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_timeout(1200)  # ждём шрифты/слайдер
...
def make_diff(baseline_path: Path, actual_path: Path, diff_path: Path):
    ...
    # Порог для отсева микрошума рендера
    mask = gray.point(lambda x: 255 if x > 12 else 0)

    # Наглядный diff (красная маска поверх actual)
    overlay = actual.copy()
